//---- Build the DOM ----
function addShadowToolbarWhenReady() {
    if (!document.body) {
        return setTimeout(addShadowToolbarWhenReady, 50);
    }
    const hostId = "soundcloud-toolbar-host";
    if (document.getElementById(hostId)) return; // to prevent duplication
    ...
    Promise.all([
        ...
        /// ==== Extension Master Settings Handler ====
        // On Load, ask for settings
        browser.runtime.sendMessage({ type: 'get-settings' }).then(settings => {
            console.log("[Settings Received from background.js]:", settings);
            applySettings(settings);
        })
        // Listen for push-based updates
        browser.runtime.onMessage.addListener((msg) => {
            if (msg.type === 'settings-updated') {
                applySettings(msg.settings);
            }
        })
        // Helper function to apply all received settings
        function applySettings(settings) {
            const startOpen = !!settings['start-open-toggle'];
            const useDarkMode = !!settings['theme-default-toggle'];
        ...
        console.error("Failed to load toolbar or styles:", err);
    });
}
addShadowToolbarWhenReady();