...
browser.tabs.query({url: "*://soundcloud.com/*"}).then(tabs => {
    for (const tab of tabs) {
        //! this is what is resetting active tab on relaod
        soundcloudTabs.set(tab.id, { title: tab.title, url: tab.url });
        browser.scripting.executeScript({
            target: { tabId: tab.id },
            files: ["content/soundcloud-controls.js"]
        }).then(() => {
            bgLog(`Manually injected controls into tab ${tab.id}`);
        }).catch(err => {
            bgWarn("Script injection failed", err);
        });
    }
...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (tab.url.includes('soundcloud.com')) {
        ...
        if (changeInfo.status === 'complete' || changeInfo.url) {
            if (activeSoundCloudTabId !== tabId) {
                ...
                browser.scripting.executeScript({
                    ...
                    bgLog(`Injected into newly loaded tab ${tabId}`);
                }).catch(err => console.warn(err));
                bgLog("SoundCloud tab set/updated and made active:", tabId, tab.url);
            }
        }
    } else {
        ...
        if (soundcloudTabs.has(tabId)) {
            ...
            bgLog("Tab navigated away, removed from SC set", tabId);
            if (tabId === activeSoundCloudTabId) {
                ...
                updateAllStatesForActiveTab();
                bgLog("Active tab removed, new active:", activeSoundCloudTabId);
            }
        }
    }
});
...
browser.tabs.onActivated.addListener(async ({ tabId }) => {
    try {
        ...
        if (tab.url && tab.url.includes("soundcloud.com") && tabId !== activeSoundCloudTabId) {
            activeSoundCloudTabId = tabId;
            updateAllStatesForActiveTab();
            bgLog("Active SC tab set by activation:", tabId);
        }
    } catch (e) {
        bgWarn(`Failed to get tab ${tabId} during activation:`, e);
    }
});
//TODO
...
browser.tabs.onRemoved.addListener(tabId => {
    if (soundcloudTabs.delete(tabId)) {
        bgLog("SoundCloud tab closed:", tabId);
    }
    if (tabId === activeSoundCloudTabId) {
        const remaining = Array.from(soundcloudTabs.keys());
        activeSoundCloudTabId = remaining.length > 0 ? remaining[0] : null;
        updateAllStatesForActiveTab();
        bgLog("Active tab closed, new active:", activeSoundCloudTabId);
    }
});
...
bgLog("Background script loaded");
// Update when you get any button messages
browser.runtime.onMessage.addListener(async (msg, sender, sendResponse) => {
    bgLog("[Background] Received message:", msg);
    ...
    if (msg.type === "get-soundcloud-tabs") {
        if (!soundcloudTabs || !(soundcloudTabs instanceof Map)) {
            console.warn("[Background] soundcloudTabs is undefined or invalid");
            return Promise.resolve({ tabs: [], error: "No soundcloudTabs map available." });
        }
        ...
        const tabs = Array.from(soundcloudTabs.entries()).map(([tabId, info]) => {
            ...
            console.debug(`[Background] Tab ID: ${tabId}`, tabObj);
            return Promise.resolve({ tabs });
        });
        console.info(`[Background] Returning ${tabs.length} SoundCloud tab(s). Active ID: ${activeId}`);
        return Promise.resolve({ tabs });
    }
...
//TODO maybe could consolidate
// Load Settings on Startup
browser.runtime.onStartup.addListener(() => {
    browser.storage.local.get(SETTINGS_KEYS).then(settings => {
        applySettingsToAllTabs();
        bgLog("[Startup] Loaded settings:", settings);
    });
});
//TODO maybe could consolidate
...
browser.runtime.onInstalled.addListener(() => {
    browser.storage.local.get(SETTINGS_KEYS).then(settings => {
        applySettingsToAllTabs();
        bgLog("[Install] Loaded settings:", settings);
    })
})
//TODO maybe could consolidate with other storage
